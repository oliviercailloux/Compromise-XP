...
\begin{enumerate}
	...
	\begin{itemize}
		\item {}[0324 (\ref{ex:0324}) or 0526 (\ref{ex:0526})] VS 0657 (\ref{ex:0657}), with drawback that 0324 and 0526 have first-half $y$.
		\item or any other $\delta = 1$ (such as 0627, \ref{ex:0627}) VS 0657 (\ref{ex:0657})
	\end{itemize}
	\item Does it matter for the FB winner to be top-ranked by a voter? 
	\begin{itemize}
		\item 0324 (\ref{ex:0324}) VS 1435 (not \ref{ex:1435} but rather the version $\set{x} \subset \BP$)
		\item 0526 (\ref{ex:0526}) VS 1637 (\ref{ex:1637}, if we choose the version $\BP = \set{x}$, not taken)
	\end{itemize}
	\item Does the best position of the FB winner matter?
	\begin{itemize}
		\item 0324 (\ref{ex:0324}) VS 3657 (\ref{ex:3657}, if we choose the version $\BP = \set{x}$)
	\end{itemize}
	\item Does $FB(P) = B(P)$ increase the propension to select the $FB$ winner? Use 0324 (\ref{ex:0324}) versus the version of 0324 where FB differs from B.
	\item Does $d(\lprof(y))$ matter, fixing $\delta$?
	\begin{itemize}
		\item{} [0546 (\ref{ex:0546}) or 1657 (\ref{ex:1657}, if we choose the version $\BP = \set{x}$)] VS 0647 (\ref{ex:0647}); or rather 0627 (\ref{ex:0627}) VS 0324 (\ref{ex:0324}) which seems better.
	\end{itemize}
	\item Do people select Borda rather than FB or MS? (Not discussed yet.)
	\item Does showing profiles in favor of MS first favor MS choices in other profiles? ⇒ To test that, we’d better randomize the order and check ex-post if there is an order effect!
	\item Does using the word “compromise” matter or using scenario  versus abstract presentation?
\end{enumerate}
As a consequence, we pick the following set of seven profiles: $\set{0324 (\ref{ex:0324}), 0526 (\ref{ex:0526}), 0657 (\ref{ex:0657}), \allowbreak 1435 (\ref{ex:1435}), \allowbreak 3657 (\ref{ex:3657}), \allowbreak 0546 (\ref{ex:0546}), 0647 (\ref{ex:0647})}$ (the example numbers are relevant only if we choose the version $\BP = \set{x}$).

\subsection{A proposal for a protocol}
The story should be an abstract setting and use the word uzlaşı for compromise. At end of the experiment we could ask the subject how she defines uzlaşı (free form text).
\commentOC{I’d also be interested to compare two or three scenarios, one of which that describes a situation where envy matters, such as choosing a restaurant for your twins, or choosing a honey moon package (with the advantage that this is not about children so not specifically favoring paternalism), though this is still under discussion.}
We do not pinpoint $x$ or $y$: subjects can choose any alternative. (This permits to check that they agree with our notion of compromise and are paying close attention, for example, a subject who chooses randomly will be detected easily as she will most likely pick alternatives that are not in $\BP \cup \FBP \cup \MSP$; still under discussion.)

Pick the seven classes of profiles indicated above, and choose, as much as possible, the version where $\BP ≠ \FBP$, to avoid priviledging FB and to check that the subject understands compromise as we expect (ie, does not prefer Borda to FB or MS).
...
%	Let $t \in T$ and $i \in N$. 
	...
	To see that $\BP = \argmin \sum \lprof = \set{x}$, observe furthermore that
