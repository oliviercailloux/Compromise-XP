...
\subsection{For m 13}
From $\max \lprof(x) ≤ \khalf$, we get $\max\lprof(x) ≤ 6$.

We get $\max\lprof(x) ≤ 13 - \max \lprof(y)$.

We get $\min \lprof(y) ≤ 12 - \max \lprof(y)$.

We get $\max\lprof(y) ≤ 8$.

From $\max \lprof(y) - \khalf + 1 ≤ \min \lprof(y)$ we get $\max \lprof(y) - 5 ≤ \min \lprof(y)$.

From $\min \lprof(y) ≤ \khalf - 1$, $\min \lprof(y) ≤ 5$.
