...
%		\item $\delta = 3 ⇔ 
	...
	\begin{itemize}
		...
		\item 0526 VS 1637
	\end{itemize}
	\item Does the position of the FB winner matter?
	\begin{itemize}
		\item 0324 VS 3657
	\end{itemize}
	\item Does $d(\lprof(y))$ matter, fixing $\Delta^\text{ineq}$?
	\begin{itemize}
		\item{} [0546 or 1657] VS 0647
	\end{itemize}
...
\subsection{Protocol}
We could run an abstract setting. We use the word compromise. (If we have enough money we can also investigate the abstract, non compromise, though Olivier is a bit skeptical about the interest of this version.)
We also might compare two or three scenarios, but we might still have to convince Ayça about this.
We do not pinpoint to $x$ or $y$, they can choose any alternative.

Towards the end of the sequence, we use a seventh prime profile, to check consistency.

We use two sequences. $S_1$ presents the highest MS-favoring profiles first, then decreasing, and $S_2$ is $S_1$ reversed. The MS-favoring profiles are determined using these criteria, lexicographically.
\begin{itemize}
...
We ask for how they choose, their rationale, for each choice we present.
...
There are $\max \lprof(x) - 1$ positions to be filled (thus positions for alternatives $z$ such that $\lprof(z)_2 ≤ \lprof(x)_2$) before $x$ (in supplement to $y$) to put $x$ at loss $\max \lprof(x)$ for the second voter. We must use for this only alternatives worst than $y$ for the first voter: othewise, thus using $z$ with $\lprof(z)_1 < \lprof(y)_1$, would, with $z$ with $\lprof(z)_2 < \lprof(y)_2$, make it dominate $y$, and otherwise, would make $z$ an FB (if $\lprof(z)_1 ≤ \lprof(x)_2$) or MS (if $\lprof(z)_1 ≥ \lprof(y)_2$) winner. There are thus only $12 - \max \lprof(y)$ available alternatives for these positions.
This constraint further excludes the profiles $((0, 6), (3, 8))$, $((0, 6), (4, 8))$ and $((1, 6), (4, 8))$.
