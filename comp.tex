...
\begin{conjecture}
	One may think that: $y \in \argmin_{\set{w \in \POP \suchthat \min\lprof(w) ≤ \rho}} (\max \lprof)$, where $y \in \argmin_{\POP} (\sigma \circ \lprof)$.
\end{conjecture}
This conjecture is FALSE. Here is the smallest counterexample.
\begin{equation}
	\begin{array}{lllll l|llll ll}
		w & \bm{x} & a	& b & c & d & e & y & f & g & h & i\\
		\scriptscriptstyle{f} & \scriptscriptstyle{g} & \scriptscriptstyle{h} & \scriptscriptstyle{i} & y & \bm{x} & w & \scriptscriptstyle{a} & \scriptscriptstyle{b} & \scriptscriptstyle{c} & \scriptscriptstyle{d} & \scriptscriptstyle{e}
	\end{array}
\end{equation}
...
If $\minineq[\FBP] > \minineq[\POP]$, $y$ is the first alt not after $x$ for 2. $y \in \argmin_{\set{w \suchthat \min\lprof(w) < \rho}} (\max \lprof)$. In other words, any $w$ with $\max \lprof(w) < y$ must satisfy $\min \lprof(w) ≥ \rho$.

Let $y \in \argmin_{\POP} (\sigma \circ \lprof)$, let $\ibar$ denote a voter such that $\lprof(x)_{\ibar} = \rho$ for some $x \in \FBP$. Then: $y \in \argmin_{\set{w \in \allalts \suchthat \lprof(w)_{\ibar} ≤ \rho}} \lprof(w)_i$.

$y \in \argmin_{\set{w \in \POP \suchthat \min\lprof(w) ≤ \rho}} (\max \lprof)$, where 
