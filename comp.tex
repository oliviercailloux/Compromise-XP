...
In particular, $y \in PE$ requires that $\sum \lprof(y) ≤ m - 1$.\footnote{Similarly, $x \in \POP$ requires that $\sum \lprof(x) ≤ m - 1$, but this latter constraint can be neglected as $\sum \lprof(x) ≤ \sum \lprof(y)$.}

For $m = 13$, we get $\min \lprof(y) ≤ 12 - \max \lprof(y)$.

From \cref{th:rhoMin}, \cref{it:rhoMin}, we know that $\forall \ibar \in \argmax \lprof(x), w \succ_i y: x \succeq_{\ibar} w$.
Also, if $\ibar \in \argmax \lprof(x)$ then $i \in \argmax \lprof(y)$ (TBD).
Thus, $\card{{\prefto_i}(y)} = \max \lprof(y)$.
We obtain that $\max \lprof(x) ≤ m - \max \lprof(y)$.
Fixing $m = 13$, we get $\max\lprof(x) ≤ 13 - \max \lprof(y)$.

It follows from $\max \lprof(x) ≤ m - \max \lprof(y)$, using $\min \lprof(x) ≥ 0$, that $(d \circ \lprof)(x) ≤ m - \max \lprof(y)$.
Also, because $\sum \lprof(y) ≤ m - 1$, we have $\min \lprof(y) ≤ m - 1 - \max \lprof(y)$, whence $(d \circ \lprof)(y) = \max \lprof(y) - \min \lprof(y) ≥ \max \lprof(y) - (m - 1 - \max \lprof(y)) = 2 \max \lprof(y) - m + 1$.
As $(d \circ \lprof)(y) < (d \circ \lprof)(x)$ is required, we obtain $2 \max \lprof(y) - m + 1 < m - \max \lprof(y)$, equivalently, $\max \lprof(y) < \frac{2 m - 1}{3}$.

Fixing $m = 13$, we get $\max\lprof(y) ≤ 8$.

Also, $\max \lprof(x) ≤ \ceil{\frac{m - 1}{2}}$. Fixing $m = 13$, we get $\max\lprof(x) ≤ 6$.

As $(d \circ \lprof)(y) < (d \circ \lprof)(x)$ is required (equivalently, $(d \circ \lprof)(y) + 1 ≤ (d \circ \lprof)(x)$), when fixing $\min \lprof(y) < \max \lprof(x) < \max \lprof(y)$, we obtain that $\min \lprof(x) ≤ \max \lprof(x) - (\max \lprof(y) - \min \lprof(y)) - 1$.

Combining the latter with $\max \lprof(x) ≤ 6$ and $0 ≤ \min \lprof(x)$, we get that $0 ≤ 5 - (\max \lprof(y) - \min \lprof(y))$, equivalently, $\max \lprof(y) - 5 ≤ \min \lprof(y)$.
...
Also, $\min \lprof(y) < \max \lprof(x)$ by ordering, and $\max \lprof(x) ≤ 6$, thus $\min \lprof(y) ≤ 5$.

By ordering, $\max \lprof(x) < \max \lprof(y)$, equivalently, $\max \lprof(x) ≤ \max \lprof(y) - 1$.

Combining the above with $\min \lprof(y) + 1 ≤ \max \lprof(x)$ yields $\min \lprof(y) + 1 ≤ \max \lprof(y) - 1$, equivalently, $\min \lprof(y) ≤ \max \lprof(y) - 2$.

Also, from $\max \lprof(y) - \min \lprof(y) + 1 ≤ \max \lprof(x) ≤ \max \lprof(y) - 1$, thus $\max \lprof(y) - \min \lprof(y) + 1 ≤ \max \lprof(y) - 1$, we get $2 ≤ \min \lprof(y)$.

Also, from $2 ≤ \min \lprof(y) ≤ \max \lprof(y) - 2$, thus $2 ≤ \max \lprof(y) - 2$, we get $4 ≤ \max \lprof(y)$.

Also, from $\max \lprof(y) - \min \lprof(y) + 1 ≤ \max \lprof(x) ≤ 13 - \max \lprof(y)$, thus $\max \lprof(y) - \min \lprof(y) + 1 ≤ 13 - \max \lprof(y)$, we get $2 \max \lprof(y) - 12 ≤ \min \lprof(y)$.

Summarizing, we get $4 ≤ \max \lprof(y) ≤ 8$.
Also, $\max \set{2, \max \lprof(y) - 5, 2 \max \lprof(y) - 12} ≤ \min \lprof(y) ≤ \min \set{12 - \max \lprof(y), \max \lprof(y) - 2, 5}$.
Also, $\max \set{\min \lprof(y) + 1, \max \lprof(y) - \min \lprof(y) + 1} ≤ \max \lprof(x) ≤ \min \set{6, \max \lprof(y) - 1, 13 - \max \lprof(y)}$ (by ordering and the lower and upper bounds here above).
And, $\min \lprof(x) ≤ \max \lprof(x) - (\max \lprof(y) - \min \lprof(y) + 1)$.
...
\begin{theorem}
	\label{th:pareto}
	Assume that $\FBP = \set{x}$ and $\argmin_{\POP} (d \circ \lprof) = \set{y}$.
	Consider any $i \in \argmax \lprof(y)$.
	Then, $x \pref_i y$ and $y \pref_{\ibar} x$.
\end{theorem}
\begin{proof}
	We know that $\max \lprof(x) < \max \lprof(y)$ from \cref{th:equiv} \cref{it:bigY}. 
...
\begin{theorem}
	\label{th:singV}
	Assume that $\FBP = \set{x}$ and $\argmin_{\POP} (d \circ \lprof) = \set{y}$.
	Then, $\argmax \lprof(x) \cap \argmax \lprof(y) = \emptyset$ (in words: the voter that maximizes $\lprof(x)$ differs from the one that maximizes $\lprof(y)$).
	Furthermore, $\forall i \in N: [i \in \argmax \lprof(y) ⇔ \ibar \in \argmax \lprof(x)]$.
\end{theorem}
\begin{proof}
	By \cref{th:equiv} \cref{it:bigY}, $\max \lprof(x) < \max \lprof(y)$.
	...
	\commentOC{I think that this point should be in the main theorem as it is useful afterwards.}
	
	Assume that $\exists i \in \argmax \lprof(x) \cap \argmax \lprof(y)$. Thus, $\lprof(x)_i = \max \lprof(x)$ and $\max \lprof(y) = \lprof(y)_i$, whence $\lprof(y)_{\ibar} = \min \lprof(y)$ and $\min \lprof(x) = \lprof(x)_{\ibar}$.
	From \cref{th:pareto}, $y \pref_{\ibar} x$.
...
\begin{corollary}
	...
	\begin{enumerate}
		...
		\item \label{it:singV} $\argmax \lprof(x) \cap \argmax \lprof(y) = \emptyset$;
		...
		\item \label{it:order} Letting furthermore $\argmax \lprof(y) = \set{i}$ (thanks to \cref{it:singV}), we have that $\lprof(x)_i = \min \lprof(x) < \lprof(y)_{\ibar} = \min \lprof(y) < \lprof(x)_{\ibar} = \max \lprof(x) < \lprof(y)_i = \max \lprof(y)$.
	\end{enumerate}
\end{corollary}
\begin{proof}
	\Cref{it:singVC} follows from (and \cref{it:singV} repeats) \cref{th:singV}.
	...
	Turning to \cref{it:order}, by definition of $i$ and $\ibar$, $\lprof(x)_{\ibar} = \max \lprof(x)$ and $\lprof(y)_i = \max \lprof(y)$, thus, $\lprof(x)_i = \min \lprof(x)$ and $\lprof(y)_{\ibar} = \min \lprof(y)$; by \cref{th:equiv} \cref{it:bigY}, $\max \lprof(x) < \max \lprof(y)$; by \cref{th:pareto}, $\lprof(y)_{\ibar} < \lprof(x)_{\ibar}$; and $\min \lprof(x) < \min \lprof(y)$ is in a proof here above (TBD!).
\end{proof}

\subsection{Borda}
\begin{theorem}
	$x \in FB(P) \suchthat 0 \in \lprof(x)(N) ⇒ B(P) \subseteq FB(P)$.
\end{theorem}
\begin{proof}
