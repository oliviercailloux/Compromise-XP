...
\begin{proof}[to be checked]
	...
	Define $A_1^{-1}$ as the inverse of $A_1$.
	Define $\pref$ as
	\begin{description}
		\item[$i$] $(a_1, …, a_m) = (A_4, x, A_5, y, A_1, A_2, A_3)$;
		\item[$\ibar$] $(A_1^{-1}, y, A_2, x, A_3, A_4, A_5)$.
	\end{description}
	...
	$\forall z \in A_4$, $\sum \lprof(z) ≥ \card{(A_1 \cup \set{y} \cup A_2 \cup \set{x} \cup A_3)} = m - t_4 + 1 ≤ t_1 + t_3$.
	Seems to fail when $t_4 ≤ \frac{2m - 3}{3}$ and $t_2 = m - 1 - t_4$ and $t_3 = m - t_4$ and $t_1 = t_3 - (t_4 - t_2 + 1)$.
	Example:
	$m = 91$; $ 4 ≤ t_4 = 59 ≤ 60$; $\set{2, 15, 22} ≤ t_2 = m - 1 - t_4 = 31 ≤ \set{31, 57, 44}$;
	$\set{32, 29} ≤ t_3 = m - t_4 = 32 ≤ \set{45, 58, 32}$;
	$t_1 = t_3 - (t_4 - t_2 + 1) = 32 - 29 = 3$;
	yields $m - t_4 + 1 = 33 ≤ t_1 + t_3 = 35$.
	
	Try to invert $A_4$. Then use $t_3 ≤ m - t_4$ thus $t_1 + t_3 ≤ t_1 + m - t_4 < t_1 + m - t_4 + 1$. But I think the plus one will not stay.
%{
%	\tiny
%	\begin{equation}
%		\begin{array}{*{16}c}
%			a_0 & … & a_{\lprof(x)_i - 1} & a_{\lprof(x)_i} = x & a_{\lprof(x)_i + 1} & … & a_{\lprof(y)_i - 1} & y & a_{\lprof(y)_i + 1} & … & a_{\sum \lprof(y) - 1} & … & a_{m - 1}\\
%			a_{\lprof(y)_i + 1} & … & a_{\sum \lprof(y)}& y & a_{\sum \lprof(y) + 1} & … &a_{\lprof(x)_{\ibar} - 1} & x & a_{\lprof(x)_i + 2} & a_{\lprof(x)_i + 1} & a_0 & … & a_{\lprof(x)_i - 2} & a_{\lprof(x)_i + 3} & … & a_{\lprof(y)_i - 1}
%		\end{array}
%	\end{equation}
%}
...
Example: $\lprof(x) = \{3, 6\}$; $\lprof(y) = \{5, 7\}$
  \begin{equation}
    \begin{array}{*{13}c}
