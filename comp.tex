...
Given $\prof \in \allprofs$ and $x \in A$, let $\lprof(x): \set{1, 2} → \intvl{0, m - 1}$ associate to each individual her loss at $x$, defined as the number of alternatives that are strictly preferred to $x$ in her preference $\prefi$, namely, $\lprof(x)_i = \card{{\prefi}(x)}$, where ${\prefi}(x)$ designate the lower contour set of $x$.
...
\begin{table}
	...
	\begin{tabular}{CCCCCC}
		\toprule
		d(\lprof(x)) & d(\lprof(y)) & \delta & v(x) & v(y) & e\\
		\midrule
		\DTLforeach*{m13}{%
			\rowdlPx=dlPx,%
			\rowdlPy=dlPy,%
		...
		\bottomrule
	\end{tabular}
	\caption{Possible values for $m = 13$; $v(z) = (\min \lprof(z), \max \lprof(z), \frac{\sum \lprof(z)}{2})$}
	\label{fig:m13}
\end{table}
