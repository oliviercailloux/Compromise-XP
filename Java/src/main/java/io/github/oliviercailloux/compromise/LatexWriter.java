...
public class LatexWriter {
  ...
  public String example(Profile profile, boolean label) {
    ...
    final String header = "$\\lprof(x) = \\{%s, %s\\}$; $\\lprof(y) = \\{%s, %s\\}$; %s"
        .formatted(lx.loss1(), lx.loss2(), ly.loss2(), ly.loss1(), profile.kind());
    final String labelStr =
        label
            ? ("\\label{ex:%s%s%s%s%s}\n".formatted(lx.loss1(), lx.loss2(), ly.loss2(), ly.loss1(),
                profile.kind()))
            : "";
    final String outer = """
        \\begin{example}[%s]
          %s%s\
        \\end{example}
        """.formatted(header, labelStr, equation.indent(2));
    return outer;
  }
}
